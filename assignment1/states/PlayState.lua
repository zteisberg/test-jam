        ...
        if not pair.scored then
            if pair.x + PIPE_WIDTH < self.bird.x then
				...
				if self.score == 2 then
					self.prize:getAchievement('third')
				elseif self.score == 8 then
					self.prize:getAchievement('second')
				elseif self.score == 15 then
					self.prize:getAchievement('first')
				end
            end
        end
...
function PlayState:enter(saved)
    -- if we're coming from death, restart scrolling
    scrolling = true
	math.randomseed(os.time())
	if saved then 
		self.bird = saved.bird
		self.pipePairs = saved.pipePairs
